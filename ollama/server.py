    ...
    if mm.backend == Backend.OLLAMA:
        ...
        try:
            with mgr.pool_lease(purpose):
                # exo's client is sync httpx and a pool generation runs for
                # minutes. Calling it directly on the event loop would stall
                # every other route, the cooldown loop and the health reporter
                # for the whole generation — and a gateway that stops reporting
                # health gets its leases revoked as stale. Hand it to a thread.
